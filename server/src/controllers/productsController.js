...
const postNewProducts = async(codigo, descripcion, talla, color, material, image, precio_base, precio_venta, preferencia, estado )=>{
    console.log('codigo--->', codigo)
    const data = await Products.findAll({where: {codigo: codigo}})
    if (data.length>0) {
        throw new Error(`Ya existe un producto con el codigo: ${codigo}`);
    }
    const newProducts = await Products.create({codigo, descripcion, talla, color, material, image, precio_base, precio_venta, preferencia, estado})
